  ...
  return (
    <div className="py-3 px-2 grid grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        ...
        {course.notes && (
          <div>
            <p className="text-[10px] text-[#6a6a8a] uppercase tracking-wider mb-1">Notes</p>
            <p className="text-xs text-[#8888a8]">{course.notes}</p>
          </div>
        )}
...
export default function CoursesPage() {
  ...
  const [filters, setFilters] = useState<Filters>({ search: "", status: "", category: "" });
  ...
  const filtered = useMemo(
    () =>
      courses
        .filter((c) => {
          if (isJunkCourse(c.id)) return false;
          if (c.countedTowardDegree === false) return false;
          if (filters.search) {
            const q = filters.search.toLowerCase();
            if (!c.number.toLowerCase().includes(q) && !c.name.toLowerCase().includes(q))
              return false;
          }
  ...
  return (
    <div className="p-8 space-y-5 max-w-5xl">
      <div>
        <h2 className="text-2xl font-bold text-[#d0d0e8]">Course Catalog</h2>
        <p className="text-[#6a6a8a] text-sm mt-1">
          Click a row to expand prereq tree and unlock graph.
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-[#1e1e34] text-[#6a6a8a] text-[10px] uppercase tracking-wider">
            <tr>
              ...
              <th className="px-4 py-3 text-center">Prereqs</th>
            </tr>
          </thead>
                ...
                <React.Fragment key={course.id}>
                  ...
                  {expanded && (
                    <tr key={`${course.id}-exp`}>
                      <td colSpan={7} className="bg-[#0d0d1e] border-t border-[#1a1a2e] px-6">
                        <CourseDetail
                          course={course}
